...
//  Sweet
//
//  Created by Mario Z. on 2018/7/9.
...
NSString *const kBeautyFragmentShaderString = SHADER_STRING
(
...
@interface CombineFilter : GPUImageFilter

@end

@implementation CombineFilter {
    CGFloat _beautyLevel;
    CGFloat _brightLevel;
    CGFloat _toneLevel;
}

- (id)init{
    self = [super initWithFragmentShaderFromString:kBeautyFragmentShaderString];
    if (self){
        _toneLevel = 0.2;
        _beautyLevel = 0.2;
        _brightLevel = 0.5;
...
@property (strong, nonatomic) CombineFilter *combineFilter;
@property (strong, nonatomic) LookupFilter *ruddyFilter;
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.combineFilter = [CombineFilter new];
        [self addFilter:self.combineFilter];
        self.ruddyFilter = [[LookupFilter alloc] initWithLookupImage:[UIImage imageNamed:@"ruddy"]];
        [self.combineFilter addTarget:self.ruddyFilter];
        self.initialFilters = @[self.combineFilter];
        self.terminalFilter = self.ruddyFilter;
    }
    return self;
}
